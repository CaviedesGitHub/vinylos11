...
class NetworkServiceAdapter constructor(context: Context) {
    ...
    fun getAlbum(albumId:Int, onComplete:(resp:VynsAlbum)->Unit, onError: (error:VolleyError)->Unit) {
        requestQueue.add(getRequest("albums/$albumId",
            Response.Listener<String> { response ->
                val resp = JSONArray(response)
                val list = mutableListOf<VynsAlbum>()
                var item:JSONObject? = null
                for (i in 0 until resp.length()) {
                    item = resp.getJSONObject(i)
                    Log.d("Response", item.toString())
                    list.add(i, VynsAlbum(id = item.getInt("id"),name = item.getString("name"), cover = item.getString("cover"), recordLabel = item.getString("recordLabel"), releaseDate = item.getString("releaseDate"), genre = item.getString("genre"), description = item.getString("description")))
                }
                onComplete(list.get(0))
            },
            Response.ErrorListener {
                onError(it)
            }))
    }
    fun getAlbums( onComplete:(resp:List<VynsAlbum>)->Unit , onError: (error:VolleyError)->Unit){
